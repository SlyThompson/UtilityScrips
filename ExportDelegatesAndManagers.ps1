...
##usage:
##.\ExportDelegatesAndManagers.ps1 -User sp3\administrator -Password ok -ExchangeServer 192.168.10.13 -CsvFile 'delegates.csv'
    ...
    if((test-path $CsvFile))
    {
        ...
        foreach($row in $csv)
        {
            ...
            foreach($d in $DelegatesArray)
            {
              ...
              if(! $Delegates.Contains($d.ToLower()))
              {
                $Delegates+=$d.Tolower();                
              }
        ...
        if($Delegates.Count -gt 0)
        {
            ...
            foreach($d in $Delegates)
            {
                ...
                LogProgress "`n[$count\$Total] Processing delegate $d";
                ...
                if($mailbox)
                {
                    ...
                    $mailboxObj | Add-Member -MemberType NoteProperty -Name UserEmail -Value $d.PrimarySmtpAddress; 
