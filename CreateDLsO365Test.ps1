...
$AllDLsOutputFolder = "$OutputFolder\AllDLsTest";
##\\Logs folder
        ...
        if(-not [string]::IsNullOrEmpty($CsvFile))
        {
		    if(test-path $CsvFile)
		    {
            	...
            	foreach($row in $csv)
            	{
                    ...
                    $EmailAddress = $EmailAddress.Replace("@myquincy.net","@DGITTech.onmicrosoft.com");
                    $Name = $row.Name;
                    ...
                    {                    
                        ...
                        if($dl)
                        {
                            ...
                            if(-not [string]::IsNullOrEmpty($ManagerEmail))
                            {
                                ...
                                Set-DistributionGroup $dl.PrimarySmtpAddress -ManagedBy mc@DGITTech.onmicrosoft.com -ErrorAction:SilentlyContinue; 
                                if($Error.Count -gt 0)
                                {
                        ...
                        if($CreateMembership)
                        {
                            ...
                            if($members.Count -gt 0)
                            {
                                ...
                                foreach($member in $members)
                                {
                                    ...
                                    $MemberEmailAddress = $MemberEmailAddress.Replace("@myquincy.net","@DGITTech.onmicrosoft.com");
