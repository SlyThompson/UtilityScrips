...
##usage:
##.\ExportPFStats.ps1 -User sp3\administrator -Password ok -ExchangeServer 192.168.10.13
...
if($Error.Count -eq 0 )
{
    ...
    {    
        ...
        foreach($pf in $PFs)
        {
            ...
            if($pf.MailEnabled)
            {
                $mPF = Get-MailPublicFolder $pf.Identity;
                $email = $mPF.PrimarySmtpAddress;
                if([string]::IsNullOrEmpty($email))
                {
                    LogInfo "PF [$($pf.Identity)] is mail-enabled but has empty email address";
                }
            }
            else
            {
                LogInfo "PF [$($pf.Identity)] is not mail enabled";
            }

            $pfObj = New-Object PSObject;
            $pfObj | Add-Member -MemberType NoteProperty -Name Identity -Value $pf.Identity; 
            $pfObj | Add-Member -MemberType NoteProperty -Name FolderType -Value $pf.FolderType; ##FolderClass for ex13+
            $pfObj | Add-Member -MemberType NoteProperty -Name MailEnabled -Value $pf.MailEnabled; ##FolderClass for ex13+
            $pfObj | Add-Member -MemberType NoteProperty -Name EmailAddress -Value $email; ##FolderClass for ex13+

            $PFsToExport.Add($pf.EntryId, $pfObj);
        ...
        {     
            ...
            {         
                ...
                @{Label = "Size (KB)";Expression = { $_.TotalItemSize.Value.ToKB()}},@{Label = "MailEnabled";Expression = { ($PFsToExport[$_.EntryId]).MailEnabled}} ,@{Label = "EmailAddress";Expression = { ($PFsToExport[$_.EntryId]).EmailAddress}},LastUserAccessTime |Export-Csv $OutputFile -NoTypeInformation;
            }
        }    
        else
        {
